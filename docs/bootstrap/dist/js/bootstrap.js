...
// NO BOOTSTRAP ACTIVE PAGE HIGHLIGHTING
...
//       // Удаление класса active у текущего активного элемента
...
signUpButton.addEventListener('click', () => {
  ...
  loginClose.classList.remove('login-close');
  loginClose.classList.add('login-close-secondary');
});

signInButton.addEventListener('click', () => {
  container.classList.remove("right-panel-active");
  loginClose.classList.remove('login-close-secondary');
...
// SYNC SLIDER WITH PRICING
var sliderValues = slider.noUiSlider.get();
minValue = parseInt(sliderValues[0]);
maxValue = parseInt(sliderValues[1]);
console.log(minValue, maxValue);

var minPriceElement = document.getElementById('min-price');
var maxPriceElement = document.getElementById('max-price');
minPriceElement.textContent = minValue * 40;
maxPriceElement.textContent = maxValue * 40;
...
function getSliderValues() {
  slider.noUiSlider.on('slide', function() {
    ...
    // updating even when not neccessary, NEEDS FIX??
    let pricing = 'От ' + minValue * 40 + ' до ' + maxValue * 40 + '₽';
    const filterPricing = document.getElementById('to-server-pricing');
    filterPricing.textContent = pricing;
  });
}
...
let typeTexts = 'Тарелки';
let materialTexts = 'Керамика';


// GET FILTERS ON SUBMIT
document.getElementById('submit-filter-button').addEventListener('click', function() {
  // GET TYPE FILTERS + DISPLAY
  const checkedTypes = document.querySelectorAll('input[name="type"]:checked');
  let typeTexts = Array.from(checkedTypes).map(input => input.nextElementSibling.textContent);
  const filterType = document.getElementById('to-server-type');
  filterType.textContent = typeTexts;

  // GET MATERIAL FILTER + DISPLAY
  const checkedMaterials = document.querySelectorAll('input[name="materials"]:checked');
  let materialTexts = Array.from(checkedMaterials).map(input => input.nextElementSibling.textContent);
  const filterMaterial = document.getElementById('to-server-materials');
  filterMaterial.textContent = materialTexts.join(', ');
});
...
// function onSubmit() {
//   document.getElementById('submit-filter-button').addEventListener('click', function() {
//     window.open("../../../source/404.html", "_self");
//   });
// }
...
document.addEventListener('DOMContentLoaded', function() {
  // CARD AUTO HEADING FILL
  const cardHeaders = document.querySelectorAll('.catalog__card-heading');
  cardHeaders.forEach((header, index) => {
    header.textContent = 'Тарелка ' + (index + 1);
  });

  // CARD AUTO DESCRIPTION FILL
  const cardDescriptions = document.querySelectorAll('.catalog__card-description');
  cardDescriptions.forEach((description, index) => {
    description.textContent = 'Мне было лень писать описание тарелки ' + (index + 1);
  });

  // CARD AUTO IMAGE FILL
  const cardContainers = document.querySelectorAll('.catalog__card-container');
